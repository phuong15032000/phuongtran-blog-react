...
export default class Social extends Component {
    ...
    retrievePosts() {
        BlogDataService.getListPostByUserId(this.state.currentUser.id)
            .then(response => {
                this.setState({
                    posts: response.data
                });
                ...
                <div className="templatemo-container">
                    ...
                    <div className="col-lg-6 col-md-6 col-sm-6 col-xs-12 white-bg right-container">
                        ...
                        <div className="tm-right-inner-container">
                            <h1 className="templatemo-header">My blogs</h1>
                            {this.state.posts.map((post, index) => (
                                <div className="row">
                                    <article className="post">
                                        <header>
                                            <div className="title">
                                                <h2><a href={"/edit/" + post.id}>{post.title}</a></h2>
                                            </div>
                                            <div className="meta">
                                        ...
                                        <a href={"/edit/" + post.id} className="image featured"><img src={post.img_thump_url} alt="" /></a>
                                        <p className="fifty-chars">{post.content}</p>
                                        <footer>
                                            <ul className="actions">
                                                <li><a href={"/edit/" + post.id} className="button large">Chỉnh sửa</a></li>
                                            </ul>
                                            <ul className="stats">
